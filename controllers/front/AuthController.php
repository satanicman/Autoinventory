...
class AuthControllerCore extends FrontController
{
    ...
    public function initContent()
    {
        ...
        if (Tools::getValue('SubmitCreate')) {
            $registration = array(
                'title' => 'Start Your Free 60 Day Trial',
                'text' => 'You will not charged for your free trial, but we require a valid credit
to pre-authorize prior to establishing service'
            );
            $this->context->smarty->assign(array(
                'registration' => $registration,
                'step' => 1
            ));
        } elseif (Tools::getValue('billingInfo') || Tools::isSubmit('submitAccount')) {
            $registration = array(
                'title' => 'Compeletion Of Registration',
                'text' => 'Please provide the following information to register your Dealership
or Car Leasing Service to be part of AutoInventory'
            );
            $this->context->smarty->assign(array(
                'registration' => $registration,
                'step' => 2,
    ...
    public function postProcess()
    {
        ...
        if (Tools::isSubmit('submitBilling')) {
            $this->processBilling();
        }
    ...
    protected function processSubmitAccount()
    {
        ...
        if (!Configuration::get('PS_REGISTRATION_PROCESS_TYPE') && !$this->ajax && !Tools::isSubmit('submitGuestAccount')) {
            if (!count($this->errors)) {
                ...
                $customer->firstname = Tools::getValue('business_name');
                $customer->business_name = $customer->firstname;
                if($id_billing_info = Tools::getValue('id')) {
                    $customer->id_billing_info = $id_billing_info;
                    $this->context->smarty->assign('id_billing', $id_billing_info);
                }
                if($business_description = Tools::getValue('business_description'))
                    $customer->business_description = $business_description;
                if($product_description = Tools::getValue('product_description'))
                    $customer->product_description = $product_description;
                if($facebook = Tools::getValue('facebook'))
                    $customer->facebook = $facebook;
                if($instagram= Tools::getValue('instagram'))
                    $customer->instagram= $instagram;
                if($twitter= Tools::getValue('twitter'))
                    $customer->twitter= $twitter;
                if($website= Tools::getValue('website'))
                    $customer->site= $website;
                ...
                if(!Tools::getValue('address'))
                    $this->errors[] = Tools::displayError('Pleas enter address.');

                if(!Tools::getValue('city'))
                    $this->errors[] = Tools::displayError('Pleas enter city.');

                if(!Tools::getValue('zip'))
                    $this->errors[] = Tools::displayError('Pleas enter zip.');

                if(!Tools::getValue('id_state'))
                    $this->errors[] = Tools::displayError('Pleas select state.');

                // New Guest customer
                $customer->is_guest = (Tools::isSubmit('is_new_customer') ? !Tools::getValue('is_new_customer', 1) : 0);
                $customer->active = 1;

                $date = new DateTime('+ ' . Configuration::get('PS_TRIAL_PERIOD') . ' day', new DateTimeZone('Europe/Kiev'));
                ...
                if (!count($this->errors)) {
                    if ($customer->add()) {
                        ...
                        if($category->add()) {
                            ...
                            Category::createManufacturer($customer->id, $category->id);
                        }
//                        if(!$_FILES['images']['error']) {
//                            copy($_FILES['images']['tmp_name'], _THEME_CAT_DIR_);
//                        }
...
//        }
        ...
        if (!$this->errors) {
            ...
            if ($card_number = Tools::getValue('card_number')) {
                $billing->card_number = preg_replace('/[^\d]/','',$card_number);
            }
