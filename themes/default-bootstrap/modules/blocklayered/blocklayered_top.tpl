...
<div id="layered-top_block_left">
	<div class="block_content">
		<form action="#" id="layered-top_form">
			<div class="filter-form">
				{*{if isset($selected_filters) && $n_filters > 0}*}
				{*<div id="enabled_filters-top">*}
				...
				{*</div>*}
				{*{/if}*}
				{foreach from=$filters_top item=filter}
					{if isset($filter.values)}
						{if isset($filter.slider)}
						<div class="layered_{$filter.type} filter-block slider" style="display: none;">
						{else}
						...
						<div class="clear"></div>
						<ul id="ul_layered-top_{$filter.type}_{$filter.id_key}">
						{if !isset($filter.slider)}
								...
								{foreach from=$filter.values key=id_value item=value name=fe}
									...
									<li class="nomargin {if $smarty.foreach.fe.index >= $filter.filter_show_limit}hiddable{/if}">
										{if isset($filter.is_color_group) && $filter.is_color_group}
											<input class="color-option {if isset($value.checked) && $value.checked}on{/if} {if !$value.nbr}disable{/if}" type="button" name="layered_{$filter.type_lite}_{$id_value}" data-rel="{$id_value}_{$filter.id_key}" id="layered-top_id_attribute_group_{$id_value}" {if !$value.nbr}disabled="disabled"{/if} style="background: {if isset($value.color)}{if file_exists($col_img_dir|cat:$id_value|cat:'.jpg')}url({$img_col_dir}{$id_value}.jpg){else}{$value.color}{/if}{else}#CCC{/if};" title="{$value.name|escape:html:'UTF-8'}" />
											{if isset($value.checked) && $value.checked}<input type="hidden" name="layered_{$filter.type_lite}_{$id_value}" value="{$id_value}" />{/if}
										{else}
											<input type="checkbox" class="checkbox" name="layered_{$filter.type_lite}_{$id_value}" id="layered-top_{$filter.type_lite}{if $id_value || $filter.type == 'quantity'}_{$id_value}{/if}" value="{$id_value}{if $filter.id_key}_{$filter.id_key}{/if}"{if isset($value.checked)} checked="checked"{/if}{if !$value.nbr} disabled="disabled"{/if} />
										{/if}
							...
							{else}
								...
								{/foreach}
								{else}
									<select class="form-control blue not_uniform medium" {if $filter.filter_show_limit > 1}multiple="multiple" size="{$filter.filter_show_limit}"{/if}>
										<option value="">{l s='Select' mod='blocklayered'} {$filter.name|escape:html:'UTF-8'}</option>
										{foreach from=$filter.values key=id_value item=value}
										{if $value.nbr || !$hide_0_values}
											<option style="color: {if isset($value.color)}{$value.color}{/if}" id="layered-top_{$filter.type_lite}{if $id_value || $filter.type == 'quantity'}_{$id_value}{/if}" value="{$id_value}_{$filter.id_key}" {if isset($value.checked) && $value.checked}selected="selected"{/if} {if !$value.nbr}disabled="disabled"{/if}>
												{$value.name|escape:html:'UTF-8'}{if $layered_show_qties}<span> ({$value.nbr})</span>{/if}</a>
											</option>
										{/if}
										{/foreach}
						...
						{else}
							{if $filter.filter_type == 0}
								<p class="filter-header">
									...
									<span class="filter-header-value medium">
										<span id="layered-top_{$filter.type}_range"></span>
									</span>
								</p>
							...
							{else}
								{if $filter.filter_type == 1}
								<li class="nomargin">
									{l s='From' mod='blocklayered'} <input class="layered_{$filter.type}_range layered_input_range_min layered_input_range" id="layered-top_{$filter.type}_range_min" type="text" value="{$filter.values[0]}"/>
									<span class="layered_{$filter.type}_range_unit">{$filter.unit}</span>
									{l s='to' mod='blocklayered'} <input class="layered_{$filter.type}_range layered_input_range_max layered_input_range" id="layered-top_{$filter.type}_range_max" type="text" value="{$filter.values[1]}"/>
									<span class="layered_{$filter.type}_range_unit">{$filter.unit}</span>
								...
								<li style="display: none;">
									<input class="layered_{$filter.type}_range" id="layered-top_{$filter.type}_range_min" type="hidden" value="{$filter.values[0]}"/>
									<input class="layered_{$filter.type}_range" id="layered-top_{$filter.type}_range_max" type="hidden" value="{$filter.values[1]}"/>
								</li>
								{/if}
							{/if}
						{/if}
	...
	<div id="layered-top_ajax_loader" style="display: none;">
		<p><img src="{$img_ps_dir}loader.gif" alt="" /><br />{l s='Loading...' mod='blocklayered'}</p>
	</div>
</div>
{else}
<div id="layered-top_block_left" class="block">
	<div class="block_content">
		<form action="#" id="layered-top_form">
			<input type="hidden" name="id_category_layered" value="{$id_category_layered}" />
		</form>
