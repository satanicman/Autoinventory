	...
	{if $nb_products > $nArray[0]}
		<form action="{if !is_array($requestNb)}{$requestNb|escape:'html':'UTF-8'}{else}{$requestNb.requestUrl|escape:'html':'UTF-8'}{/if}" method="get" class="nbrItemPage">
			<div class="product-count">
				...
				{/if}
				{if $nb_products > 1}
					{l s='%1$d - %2$d of %3$d vehicles showing' sprintf=[$productShowingStart, $productShowing, $nb_products]}
				{else}
					{l s='%1$d - %2$d of 1 vehicl showing' sprintf=[$productShowingStart, $productShowing]}
				{/if}
			</div>
			<div class="clearfix selector1">
				...
				{if is_array($requestNb)}
					...
					{/foreach}
				{/if}
				<select name="n" id="nb_item{if isset($paginationId)}_{$paginationId}{/if}" class="form-control not_uniform">
					{assign var="lastnValue" value="0"}
					{foreach from=$nArray item=nValue}
						{if $lastnValue <= $nb_products}
							<option value="{$nValue|escape:'html':'UTF-8'}" {if $n == $nValue}selected="selected"{/if}>{$nValue|escape:'html':'UTF-8'} {l s='per page'}</option>
						{/if}
