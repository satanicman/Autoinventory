...
{capture name=path}<a href="{$link->getPageLink('my-account.php', true)|escape:'htmlall':'UTF-8'}">{l s='My account' mod='addprod'}</a><span class="navigation-pipe">{$navigationPipe|escape:'htmlall':'UTF-8'}</span>{l s='My products' mod='addprod'}{/capture}
{include file="$tpl_dir./breadcrumb.tpl"}

<script type="text/javascript" src="{$module_dir|escape:'htmlall':'UTF-8'}ui/js/jquery-ui-1.8.22.custom.min.js"></script>
<script type="text/javascript" src="{$module_dir|escape:'htmlall':'UTF-8'}views/js/jquery-1.11.3.min.js"></script>
<script>
   var jq111 = jQuery.noConflict( true );
</script>
<link rel="stylesheet" type="text/css" href="{$module_dir|escape:'htmlall':'UTF-8'}ui/css/ui-darkness/jquery-ui-1.8.22.custom.css" />
<link rel="stylesheet" type="text/css" href="{$module_dir|escape:'htmlall':'UTF-8'}views/css/addpro14.css" />
...
{/if}
{if isset($r_deleted)}
    <p class="success alert alert-success">{l s='Product has been removed!' mod='addprod'}</p>
{elseif isset($r_updated)}
    <p class="success alert alert-success">{l s='This product is updated' mod='addprod'}{if !$upd_mod} {l s=' and sent for moderation.' mod='addprod'}{/if}</p>
{elseif isset($r_added)}
    <p class="success alert alert-success">{l s='Product added' mod='addprod'}{if !$add_mod} {l s=' and sent for moderation.' mod='addprod'}{/if}</p>
{/if}
{if isset($sNewMessageAboutW) && $sNewMessageAboutW != ''}
    <p class="success alert alert-success">{l s=$sNewMessageAboutW mod='addprod'}</p>
{/if}
<ul class="idTabs">
	...
	<form method="post" action="{$base_dir_ssl|escape:'htmlall':'UTF-8'}modules/addprod/addprod-user.php" class="std" enctype="multipart/form-data">
            
	    <fieldset style="width:100%;" class="block corners">
		{if isset($eproduct->id)}<input name="product_id" type="hidden" value="{$eproduct->id|intval}" />{/if}
                ...
                <div style="margin:12px;">
                    ...
                    {if isset($categories.children) && !empty($categories.children)}
                        ...
                        {foreach from=$categories.children item=category}
                            <option class="categories_t" value="{$category.id|intval}"  {if isset($post.categories) && in_array($category.id, $post.categories)} selected{else}{if isset($aProductCategoriesIds) && in_array($category.id, $aProductCategoriesIds)} selected{/if}{/if} attr-name='{$category.name|escape:'html':'UTF-8'}'>{$category.name|escape:'html':'UTF-8'}</option>                            
		    ...
		    <select name="defaultCategory" id='defaultCategory'>
                        {if !empty($aProductCategories) && $aProductCategories}
                            {foreach from=$aProductCategories item=categoryT}
                                <option value="{$categoryT->id|intval}" {if isset($post.defaultCategory) && $post.defaultCategory == $categoryT->id}selected{else}{if isset($eproduct->id) && $categoryT->id == $eproduct->id_category_default}selected{/if}{/if}>{$categoryT->name|escape:'html':'UTF-8'}</option>
                            {/foreach}
                        {/if}
		    </select>
		</div>
		    ...
		    <select name="tax_rule" id='tax_rule'>
                        ...
                        {if !empty($aTaxRules) && $aTaxRules}
                            {foreach from=$aTaxRules item=a_tax}
                                <option value="{$a_tax.id_tax_rules_group|intval}" {if isset($post.tax_rule) && $post.tax_rule == $a_tax.id_tax_rules_group}selected{else}{if isset($eproduct->id) && $a_tax.id_tax_rules_group == $eproduct->id_tax_rules_group}selected{/if}{/if}>{$a_tax.name|escape:'html':'UTF-8'}</option>
                            {/foreach}
                        {/if}
		    </select>
		</div>
		...
		<div style="margin:12px;">
			...
			<input type="text" name="price" class="form-txt form-small" value="{if isset($post.price) && $post.price}{$post.price|escape:'htmlall':'UTF-8'}{else}{if isset($eproduct->price)}{$eproduct->price|escape:'htmlall':'UTF-8'}{else}0{/if}{/if}" size="7" /> {$currency_sign|escape:'htmlall':'UTF-8'}
		</div>
		...
		<div id="tabs">
			<ul style="margin:12px;">
				{foreach from=$lngs item=lang}
				<li>
					<a href="#tabs-{$lang.id_lang|intval}"><img src="{$img_ps_dir|escape:'htmlall':'UTF-8'}l/{$lang.id_lang|intval}.jpg" alt="{$lang.iso_code|escape:'htmlall':'UTF-8'}" title="{$lang.name|escape:'htmlall':'UTF-8'}" /></a>
				</li>
				{/foreach}
			</ul>
			{foreach from=$lngs item=lang}
			<div id="tabs-{$lang.id_lang|intval}">
				<div style="margin:12px;">
				    ...
				    <input type="text" class="form-txt" name="name_{$lang.id_lang|intval}" value="{if isset($lang.posted_name) && $lang.posted_name != ''}{$lang.posted_name|escape:'htmlall':'UTF-8'}{else}{if isset($lang.product->name)}{$lang.product->name|escape:'htmlall':'UTF-8'|stripslashes}{/if}{/if}" placeholder="{l s='Product name' mod='addprod'}" size="48" />
				</div>
				<div>
				    <p  style="margin:8px;">{l s='Short description' mod='addprod'}:</p>
				    <textarea id="description_short_{$lang.id_lang|intval}" name="description_short_{$lang.id_lang|intval}" rows="5" cols="65" placeholder="{l s='Short description' mod='addprod'}">{if isset($lang.posted_s_descr) && $lang.posted_s_descr != ''}{$lang.posted_s_descr}{else}{if isset($lang.product->description_short)}{$lang.product->description_short|escape:'htmlall':'UTF-8'|stripslashes}{/if}{/if}</textarea>
				</div>
				<div style="width:100%; margin:9px;">
				    <p>{l s='Full description' mod='addprod'}:</p>
				    <textarea id="description_{$lang.id_lang|intval}" name="description_{$lang.id_lang|intval}" rows="10" cols="45">{if isset($lang.posted_description) && $lang.posted_description != ''}{$lang.posted_description}{else}{if isset($lang.product->description)}{$lang.product->description|escape:'htmlall':'UTF-8'|stripslashes}{/if}{/if}</textarea>
				</div>
			</div>
			{/foreach}
			<div style="margin:12px;">
		...
		<ul id="accessories">
					{foreach from=$eimages item=eimage}
						<li>
							...
							<img src="{$link->getImageLink($eproduct->link_rewrite, $imageIds, 'medium')|escape:'htmlall':'UTF-8'}" height="{if isset($mediumSize.height) && $mediumSize.height}{$mediumSize.height|escape:'htmlall':'UTF-8'}{/if}" />
<div class="hover_descr">
<h5><a href="{$base_dir_ssl|escape:'htmlall':'UTF-8'}modules/addprod/addprod-user.php?delimg={$eimage.id_image|intval}&edit={$eproduct->id|intval}">{l s='Delete' mod='addprod'}</a></h5>
</div>
						</li>
					{/foreach}
		</ul>
	...
	{/if}
		...
		<p>{l s='Product file' mod='addprod'}: <b>{$filename|escape:'htmlall':'UTF-8'}</b></p>
		<p class="warning" style="padding:6px 12px;">{l s='Do not upload the file if you want to leave it unchanged.' mod='addprod'}</p>
        {/if}
		</div>
		<div style="margin:12px;">
		    <label for="image_product" class="form-lbl" style="float: left; width: 150px;">{l s='Image' mod='addprod'}</label>
	...
	<table class="table" style="width: 100%;">
	    ...
	    {foreach from=$products item=product}
            ...
            <tr>
                ...
                <td width="15%">
                <a href="{$base_dir_ssl|escape:'htmlall':'UTF-8'}modules/addprod/addprod-user.php?edit={$product.id_product|intval}"><img src="{$img_ps_dir|escape:'htmlall':'UTF-8'}admin/edit.gif" /></a>
                <a href="{$base_dir_ssl|escape:'htmlall':'UTF-8'}modules/addprod/addprod-user.php?delete={$product.id_product|intval}"><img src="{$img_ps_dir|escape:'htmlall':'UTF-8'}admin/delete.gif" /></a>
                </td>
            </tr>
            {else}
            <tr>
    ...
    <div id="idTab3">
	<div class="rte">
	    <p class="info" style="padding:12px;">
	...
	<form method="post" action="{$base_dir_ssl|escape:'htmlall':'UTF-8'}modules/addprod/addprod-user.php" class="std">
	    <fieldset class="block corners"  style="padding:12px;">
		<label style="width:100%;"><b>{l s='Withdraw query' mod='addprod'}</b></label></br>
		<p>
		    <label for="summ" class="form-lbl">{l s='Amount' mod='addprod'}:</label>
		    <input type="text" class="form-txt" name="summ" placeholder="{l s='Amount' mod='addprod'}" size="7" /> {$currency_sign|escape:'htmlall':'UTF-8'}
		</p>
		<p>
		    <label for="message" class="form-lbl">{l s='Comment' mod='addprod'}:</label>
    ...
    {/if}
        ...
        <div id="idTab4">
            ...
            <div>
                ...
                <div class="preview_logo">
                    {if file_exists("`$smarty.const._PS_IMG_DIR_`m/`$manufacturer->id`.jpg")}
        ...
        <script>
            ...
            $('#logo').live('change', function () {
                file_reader.readAsDataURL($(this).get(0).files[0]);
            });
            ...
            $('#saveManufacturerInfo').live('click', function () {
                ...
                jq111.ajax({
                    url: document.location.href + '?ajax=true&saveManufacturerInfo=true',
                    type: 'POST',
                    dataType: 'json',
                    processData: false,
                    contentType: false,
