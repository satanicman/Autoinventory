...
//    custom #END
    ...
    public static $definition = array(
        ...
        'fields' => array(
            // custom
            'business_name' =>                  array('type' => self::TYPE_STRING, 'validate' => 'isName', 'required' => true, 'size' => 128),
            'id_billing_info' =>                array('type' => self::TYPE_INT, 'validate' => 'isUnsignedId'),
            'business_description' =>           array('type' => self::TYPE_HTML, 'validate' => 'isCleanHtml'),
            'product_description' =>            array('type' => self::TYPE_HTML, 'validate' => 'isCleanHtml'),
            'facebook' =>                       array('type' => self::TYPE_STRING, 'validate' => 'isReference'),
            'instagram' =>                      array('type' => self::TYPE_STRING, 'validate' => 'isReference'),
        ...
        'associations' => array(
            'billing_info' =>                   array('type' => self::HAS_ONE)
        ),
    );
    ...
    public function delete()
    {
        ...
        Db::getInstance()->execute('DELETE FROM '._DB_PREFIX_.'phones WHERE id_customer='.(int)$this->id);
        Db::getInstance()->execute('DELETE FROM '._DB_PREFIX_.'customer_offers WHERE id_customer='.(int)$this->id);
        Db::getInstance()->execute('DELETE FROM '._DB_PREFIX_.'customer_time WHERE id_customer='.(int)$this->id);
        Db::getInstance()->execute('DELETE FROM '._DB_PREFIX_.'billing_info WHERE id_billing_Info='.(int)$this->id_billing_info);
    ...
    */
    ...
    public function getBilling($id_billing_Info = null)
    {
        if(!$id_billing_Info)
            $id_billing_Info = $this->id_billing_info;

        if(!$id_billing_Info)
            return false;

        $sql = 'SELECT *
				FROM `'._DB_PREFIX_.'billing_info`
				WHERE 1 AND `id_billing_Info` = ' . $id_billing_Info . '
				ORDER BY `id_billing_Info` ASC';
    ...
    public function setOffers($offers) {
        ...
        Db::getInstance()->query('
			DELETE FROM '._DB_PREFIX_.'customer_offers
			WHERE `id_customer` = '.(int)$this->id
        );
        foreach ($offers as $offer) {
            $row = array('id_customer' => (int)$this->id, 'id_offers' => (int)$offer);
            Db::getInstance()->insert('customer_offers', $row);
        }
    }

    public function getOffers() {
        $sql = 'SELECT o.* 
                FROM '._DB_PREFIX_.'customer_offers co
                LEFT JOIN '._DB_PREFIX_.'offers as o ON co.id_offers = o.id_offers
                WHERE co.`id_customer` = '.(int)$this->id;
        return Db::getInstance()->executeS($sql);
    }
    ...
    public function setTimes($times) {
        ...
        Db::getInstance()->query('
			DELETE FROM '._DB_PREFIX_.'customer_time
			WHERE `id_customer` = '.(int)$this->id
        );
        foreach ($times as $type => $days) {
            foreach($days as $name => $day) {
                $row = array(
                    ...
                    'to' => (string)$day['to'],
                    'closed' => (isset($day['closed']) && $day['closed']) ? 1 : 0,
                );

                Db::getInstance()->insert('customer_time', $row);
            }
        }
    }

    public function getTime() {
        $times = Db::getInstance()->executeS('SELECT * FROM '._DB_PREFIX_.'customer_time WHERE `id_customer` = '.(int)$this->id.' ORDER BY id_time ASC');
        $t = array();
        foreach ($times as $time) {
            $t[$time['type']][$time['day']] = array(
                'from' => $time['from'],
                'to' => $time['to'],
                'closed' => $time['closed'],
            );
    ...
    public function getPhones() {
        return Db::getInstance()->executeS('SELECT * FROM '._DB_PREFIX_.'phones WHERE `id_customer` = '.(int)$this->id);
    }
}
